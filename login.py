...
from check2 import FaceIdentificationSystem
import pytz
# Function part
def login_use():
    ...
    if row==None:
        ...
        home_page()
def hide():
    openeye.config(file='.vscode/images/closeye.png')
    Password.config(show='*')
    eyeButton.config(command=show)
def show():
    openeye.config(file='.vscode/images/openeye.png')
    Password.config(show='')
...
bgImage=ImageTk.PhotoImage(file='.vscode/images/bg7.webp')
bg4Label=Label(rootlw,image=bgImage)
bg4Label.grid()
frame=Frame(rootlw,bg='deepskyblue4')
frame.place(x=575,y=150)
heading=Label(frame,text='USER LOGIN',font=('calibri',31,'bold'),bg='deepskyblue4'
              ,fg='white')
...
openeye=PhotoImage(file='.vscode/images/openeye.png')
...
sign_btn.grid(row=16,column=0,sticky='w',padx=25,pady=(10,0))
newa_but = Button(frame,text="Create New", font=('open sans', 12, 'bold underline'),fg='orange',bg='deepskyblue4',cursor='hand2',activeforeground='white',activebackground='firebrick1',bd=0,command=sign_page)
