...
def clear():
    ...
    check.set(0)
    # student_window = tk.Toplevel(signup_window)
    # student_app = Student(student_window)
    signup_window.destroy()
    import adminLogin
def connect_database():
    if (emailEntry.get()=='' or userEntry.get()=='' or PassEntry.get()=='' or ConfirmEntry.get()==''):
        ...
        try:
            ...
            return
        try:
            query='create database admininfo;'
            ...
            mycursor.execute('use admininfo')
...
heading=Label(frame,text='Admin Account',font=('calibri',31,'bold'),bg='deepskyblue4'
              ,fg='white')
...
account_label = Label(frame, text="Have an account", font=('open sans', 9, 'bold'),fg='white',bg='deepskyblue4')
account_label.grid(row=13, column=0,sticky='w',padx=25,pady=10) # Notice row 11 instead of 10

login_but = Button(frame, text="LOG IN", font=('open sans', 9, 'bold underline'), bd=0,fg='yellow',bg='deepskyblue4',activeforeground='blue',cursor='hand2',command=admin_page)
