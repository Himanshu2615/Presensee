...
from Update_img import manage_employee_folder, capture_and_save_images, get_emp_details_from_user
class Student:
    def __init__(self, root):
        ...
        update_btn = Button(btn1_frame, text="Update Photo", width=34, font=("times new roman ", 13, "bold"), bg="blue", activebackground="orange",command=self.update_img, fg="white")
        update_btn.grid(row=0, column=1)
        ...
        self.Search_box["values"] = ("Select ", "Employee_ID", "Phone_No")
...
#-----------------------fetch data------------
    def fetch_data(self):
        conn = pymysql.connect(host="localhost", user="root", password="P2615", database="studentinfo")
    ...
    def open_photo_capture(self):
        name, emp_id = image_capture.get_emp_details_from_user()
        if name and emp_id:
            image_capture.capture_and_save_images(name, emp_id)
        else:
    ...
    def update_img(self):
        ...
        if emp_id:
        # Manage the folder: delete if it exists, otherwise show ID not found
            if manage_employee_folder(emp_name, emp_id):
            # If folder was managed successfully, proceed to capture images
